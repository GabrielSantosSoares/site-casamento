...
import { ManualFuncao } from "./ManualFuncao";
import { Dashboard, DashboardNoivos } from "./DashboardNoivos";
import {
...
export type Invitation = {
  ...
  aviso_eleicoes_lido?: boolean;
  evento: EventInfo;
};

type Props = {
  eventoInicial: EventInfo;
  acessoInicial?: {
    convite: Invitation;
    presentes: Presente[];
  ...
  somenteGrupoInicial?: boolean;
  pagamentoInicial?: string | null;
};
...
function retornoPagamentoPorStatus(status?: string | null): RetornoPagamento | null {
  if (status === "sucesso")
    return {
      status,
      mensagem:
...
function ehPerfilDaOrganizacao(perfil?: Invitation["perfil_acesso"]) {
  return ["noivos", "assessoria", "organizacao", "admin"].includes(
    perfil ?? "",
  );
}

function nomeDaArea(perfil?: Invitation["perfil_acesso"]) {
  if (perfil === "admin") return "Área da Administração";
  if (perfil === "assessoria") return "Área da Assessoria";
  if (perfil === "organizacao") return "Área da Organização";
  return "Área dos Noivos";
}

function visualizacaoInicial(convite?: Invitation | null): View {
  if (!convite) return "inicio";
  if (ehPerfilDaOrganizacao(convite.perfil_acesso)) return "noivos";
  return convite.convidados.some((pessoa) => pessoa.status === "sim")
    ? "inicio"
    : "convite";
}

export default function AplicacaoCasamento({
  eventoInicial,
  acessoInicial = null,
  codigoInicial = "",
  somenteGrupoInicial = false,
  pagamentoInicial = null,
}: Props) {
  const conviteInicial = acessoInicial?.convite ?? null;
  const retornoInicial = retornoPagamentoPorStatus(pagamentoInicial);
  const [guest, setGuest] = useState(Boolean(conviteInicial));
  const [code, setCode] = useState(codigoInicial);
  const [view, setView] = useState<View>(
    retornoInicial && !ehPerfilDaOrganizacao(conviteInicial?.perfil_acesso)
      ? "presentes"
      : visualizacaoInicial(conviteInicial),
  );
  const [confirmed, setConfirmed] = useState(false);
  const [notice, setNotice] = useState("");
  const [invitation, setInvitation] = useState<Invitation | null>(conviteInicial);
  const [responses, setResponses] = useState<
    Record<string, "sim" | "nao" | null>
  >(
    Object.fromEntries(
      (conviteInicial?.convidados ?? []).map((pessoa) => [
        pessoa.id,
        pessoa.status,
      ]),
    ),
  ...
  const [idades, setIdades] = useState<Record<string, string>>(
    Object.fromEntries(
      (conviteInicial?.convidados ?? []).map((pessoa) => [
        pessoa.id,
        pessoa.idade === null ? "" : String(pessoa.idade),
      ]),
    ),
  );
  ...
  const [presentes, setPresentes] = useState<Presente[]>(
    acessoInicial?.presentes ?? [],
  );
  const [carrinho, setCarrinho] = useState<Record<string, number>>({});
  const [presentesConfirmados, setPresentesConfirmados] = useState(
    retornoInicial?.status === "sucesso",
  );
  const [mercadoPagoDisponivel, setMercadoPagoDisponivel] = useState(
      Boolean(acessoInicial?.mercado_pago_disponivel),
    ),
  ...
  const [eventoPublico] = useState<EventInfo>(eventoInicial);
  const [grupoSomenteLeitura, setGrupoSomenteLeitura] = useState(
      somenteGrupoInicial,
    ),
    [codigoGestor, setCodigoGestor] = useState("");
  const [qrConvite, setQrConvite] = useState("");
  const [retornoPagamento, setRetornoPagamento] =
    useState<RetornoPagamento | null>(retornoInicial);
  const [avisoEleicoesAberto, setAvisoEleicoesAberto] = useState(false);
  ...
  const acessarCodigo = useCallback(
    async (codigoAcesso: string, somenteGrupo = false) => {
      ...
      try {
        ...
        const organizacao = ehPerfilDaOrganizacao(found.perfil_acesso);
        const temPresencaConfirmada = found.convidados.some(
          (person) => person.status === "sim",
        );
        const retorno = retornoPagamentoPorStatus(
  ...
  useEffect(() => {
    if (conviteInicial && codigoInicial) {
      try {
        window.sessionStorage.setItem(
          "casamento_codigo_ativo",
        ...
        // A rota continua contendo o código quando o armazenamento está indisponível.
      }
      if (conviteInicial.perfil_acesso === "convidado")
        queueMicrotask(() => void carregarHistoricoPorCodigo(codigoInicial));
      void QRCode.toDataURL(
        `${window.location.origin}/${somenteGrupoInicial ? "g" : "c"}/${codigoInicial}`,
        {
          width: 280,
          margin: 1,
          color: { dark: "#53604f", light: "#fffdf9" },
      ...
      if (retornoInicial)
        window.history.replaceState(
          {},
          "",
          `/${somenteGrupoInicial ? "g" : "c"}/${codigoInicial}`,
        );
      return;
    }
  ...
  async function liberarGrupo(event: FormEvent) {
    ...
    try {
      ...
      setResponses(
        Object.fromEntries(
          found.convidados.map((person) => [person.id, person.status]),
        ),
      );
      setIdades(
        Object.fromEntries(
  ...
  function abrirGerenciamentoPresenca() {
    setNotice("");
    if (!invitation?.aviso_eleicoes_lido) {
      setAvisoEleicoesAberto(true);
      return;
    }
    setView("presenca");
  }

  async function confirmarLeituraDoAviso() {
    if (!invitation) return;
    setLoading(true);
    setNotice("");
    try {
      const resposta = await fetch("/api/convite", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          action: "confirmar_leitura_aviso_eleicoes",
          codigo: invitation.codigo,
        }),
      });
      const dados = await resposta.json();
      if (!resposta.ok)
        throw new Error(dados.error || "Não foi possível registrar a leitura.");
      setInvitation({ ...invitation, aviso_eleicoes_lido: true });
      setAvisoEleicoesAberto(false);
      setView("presenca");
    } catch (erro) {
      setNotice(
  ...
  async function confirm(event: FormEvent) {
    ...
    const semResposta = invitation.convidados.filter(
      (pessoa) => responses[pessoa.id] !== "sim" && responses[pessoa.id] !== "nao",
    );
    if (semResposta.length) {
      setNotice(
        `Escolha “Estarei presente” ou “Não poderei ir” para ${semResposta.length === 1 ? semResposta[0].nome : "todas as pessoas do convite"}.`,
      );
      return;
    }
    setLoading(true);
      ...
      const response = await fetch("/api/convite", {
        ...
        body: JSON.stringify({
          ...
          respostas: invitation.convidados.map((person) => ({
            ...
            status: responses[person.id] as "sim" | "nao",
            idade:
    ...
    <main className="guest-app">
      {avisoEleicoesAberto && (
        <div className="gift-modal-backdrop" role="presentation">
          <section
            className="gift-modal election-notice"
            role="dialog"
            aria-modal="true"
            aria-labelledby="election-notice-title"
          >
            <span className="gift-modal-icon">♡</span>
            <p className="eyebrow">Um pedido especial</p>
            <h2 id="election-notice-title">Aos nossos convidados:</h2>
            <p>
              Como nosso casamento será{" "}
              <strong className="election-highlight">
                na véspera das eleições
              </strong>
              , pedimos que confirme sua presença apenas se realmente puder
              comparecer. Sua confirmação nos ajudará muito na organização
            ...
            <button
              className="primary"
              onClick={() => void confirmarLeituraDoAviso()}
              disabled={loading}
            >
              {loading ? "Registrando..." : "Li e entendi"}
            </button>
          </section>
        </div>
      )}
      <header className="guest-header">
        ...
        <nav aria-label="Área do convidado">
          {(ehPerfilDaOrganizacao(invitation?.perfil_acesso)
            ? []
            : grupoSomenteLeitura
              ? ["convite" as View]
              : ([
        ...
        {(invitation?.nivel_acesso ?? 0) >= 3 && (
          ...
          >
            {nomeDaArea(invitation?.perfil_acesso)}
          </button>
        )}
        ...
        <div className="guest-content">
          {view === "inicio" && (
            <div className="welcome">
              ...
              <section
                className="quick-event-info"
                aria-label="Informações rápidas do casamento"
              >
                <div>
                  <small>Data e horário</small>
                  <b>
                    {dataEvento.dataExtenso} · {dataEvento.hora}
                  </b>
                </div>
                <div>
                  <small>Local</small>
                  {invitation?.evento?.local_liberado ? (
                    <>
                      ...
                      <span>{invitation.evento.endereco}</span>
                    ...
                    <span>O endereço será disponibilizado pela organização.</span>
                  )}
                </div>
              </section>
            </div>
                    ...
                    <form onSubmit={confirm}>
                      ...
                      {invitation?.convidados.map((person) => (
                        <div className="person-row" key={person.id}>
                          ...
                          {person.crianca && responses[person.id] === "sim" && (
                            <input
              ...
              <div className="panel couple-login">
                ...
                <h1>
                  {invitation?.senha_criada
                    ? `Entrar na ${nomeDaArea(invitation?.perfil_acesso)}`
                    : `Crie a senha de acesso — ${nomeDaArea(invitation?.perfil_acesso)}`}
                </h1>
                <p>
                  {invitation?.senha_criada
